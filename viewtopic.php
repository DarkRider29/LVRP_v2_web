...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul class="crumbs">
			<li><a href="forum.php"><?php echo $lang_common['Index'] ?></a></li>
...
while ($cur_post = $db->fetch_assoc($result))
{
	...
	if ($cur_post['poster_id'] > 1)
	{
		if ($pun_user['g_view_users'] == '1')
			$username = '<center><a href="profile.php?id='.$cur_post['poster_id'].'</center>">'.pun_htmlspecialchars($cur_post['username']).'</a>';
		else
			$username = pun_htmlspecialchars($cur_post['username']);

		if($cur_post['g_id']==1)
			{$user_title='<img height="30" width="120" src="img/groups/fonda.png"/>';}
		elseif($cur_post['g_id']==2)
			{$user_title='<img height="30" width="120" src="img/groups/moderateur.png"/>';}
		elseif($cur_post['g_id']==4)
			{$user_title='<img height="30" width="120" src="img/groups/joueur.png"/>';}
		elseif($cur_post['g_id']==5)
			{$user_title='<img height="30" width="120" src="img/groups/Co-fonda.png"/>';}
		elseif($cur_post['g_id']==6)
			{$user_title='<img height="30" width="120" src="img/groups/gestionnaire.png"/>';}
		elseif($cur_post['g_id']==8)
			{$user_title='<img height="30" width="120" src="img/groups/admin.png"/>';}
		elseif($cur_post['g_id']==17)
			{$user_title='<img height="30" width="120" src="img/groups/helpeur.png"/>';}
		else
		...
		$is_online = ($cur_post['is_online'] == $cur_post['poster_id']) ? '<img src="images/user_online.gif"/> <strong>'.$lang_topic['Online'].'</strong>' : '<img src="images/user_offline.gif"/> <span>'.$lang_topic['Offline'].'</span>';
		...
		if ($pun_config['o_show_user_info'] == '1')
		{
			...
			if ($pun_config['o_show_post_count'] == '1' || $pun_user['is_admmod'])
				...
				/*$user_info[] = '<br/><dd><span><center>Informations Perso</center></span></dd>';
				$user_info[] = '<dd><span>Faction : '.get_FacName($dStats['Member']).' ('.get_FacRank($dStats['Member'],$dStats['Rank']).') </span></dd>';
				$user_info[] = '<dd><span>Level : '.$dStats['Level'].' </span></dd>';
				if($dStats['Warnings']>0)
					{$user_info[] = '<dd><span>Avertissements : '.$dStats['Warnings'].' </span></dd>';}
				if($dStats['DonateRank']>0)
					{$user_info[] = '<dd><span>VIP : Oui </span></dd>';}
				mysql_free_result();*/
		...
		{
			$user_info[] = '<br/><dd><span><a href="moderate.php?get_host='.$cur_post['id'].'" title="'.$cur_post['poster_ip'].'">'.$lang_topic['IP address logged'].'</a></span></dd>';
...
<?php if ($user_avatar != '') echo "\t\t\t\t\t\t".'<dd class="postavatar"><center>'.$user_avatar.'</center></dd>'."\n"; ?>
						<dd class="usertitle"><strong><center><?php echo $user_title ?></center></strong></dd>
...
?>
<textarea class="editor full" id="req_message" name="req_message" rows="7" cols="75" tabindex="<?php echo $cur_index++ ?>"></textarea></label>
<?php /* FluxToolBar */
