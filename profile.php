...
if ($id < 2 && (($action != 'upload_avatar' || $action != 'upload_avatar2') && $id != 0))
...
// Load the language file for the mod Easy Avatar
if (file_exists(PUN_ROOT.'lang/'.$pun_user['language'].'/EZavatar.php'))
	require PUN_ROOT.'lang/'.$pun_user['language'].'/EZavatar.php';
else
	require PUN_ROOT.'lang/English/EZavatar.php';

// Easy Avatar : check image types supported by the GD library (if it's loaded...)
$gd_image_types_ability = FALSE;
if (extension_loaded('gd'))
{
	$supported_image_types = array();
	$possible_image_types = array(
		IMG_GIF	=> '.gif', 
		IMG_JPG	=> '.jpg', 
		IMG_PNG	=> '.png'
	);
	
	foreach ($possible_image_types as $cur_image_type => $cur_extension)
	{
		if (imagetypes() & $cur_image_type)
		{
			$supported_image_types[] = $cur_extension;
			$gd_image_types_ability = TRUE;
		}
		else
...
// Easy Avatar : make sure avatars_upload_size doesn't exceed post_max_size set in config.ini and decrease it if needed (default : 1Mb)
if ($max = trim(@ini_get('post_max_size')))
{
	preg_match('#([0-9]+)[\s]*([a-z]+)#i', $max, $matches);
	$last = '';
	if(isset($matches[2]))
		$last = $matches[2];
	if(isset($matches[1]))
...
if (intval($pun_config['avatars_upload_size']) > $max)
{
	$pun_config['avatars_upload_size'] = $max;
	$db->query('UPDATE '.$db->prefix.'config SET conf_value='.$max.' WHERE conf_name=o_avatars_upload_size') or error('Unable to set avatars_upload_size', __FILE__, __LINE__, $db->error());
}
	
// Easy Avatar : reducing image function
function reduce_image($image_to_reduce)
{
	global $pun_config, $lang_EZavatar, $width, $height, $size;
	
	if ($pun_config['o_avatars_width'] < 1 || $pun_config['o_avatars_height'] < 1 || $pun_config['o_avatars_size'] < 1)
		message($lang_EZavatar['Zero set'].'<a href="mailto:'.$pun_config['o_admin_email'].'">'.$pun_config['o_admin_email'].'</a>.');
		
	$ratio = max($width/$pun_config['o_avatars_width'], $height/$pun_config['o_avatars_height'], sqrt($size/$pun_config['o_avatars_size']));
	$reduced_width = intval($width/$ratio);
	$reduced_height = intval($height/$ratio);
	
	if ($reduced_width < 1 || $reduced_height < 1)
		message ($lang_EZavatar['Ratio too high'].'<a href="mailto:'.$pun_config['o_admin_email'].'">'.$pun_config['o_admin_email'].'</a>.');
		
	if (!($reduced_image = @imagecreatetruecolor($reduced_width, $reduced_height)))
		message($lang_EZavatar['Create image problem']);

	if (@imagecopyresampled($reduced_image, $image_to_reduce, 0, 0, 0, 0, $reduced_width, $reduced_height, $width, $height))
		return $reduced_image;
	else
		message($lang_EZavatar['Reduction problem']);
}
...
if ($action == 'change_pass')
{
	...
	if (isset($_POST['form_sent']))
	{
		...
		$db->query('UPDATE lvrp_users SET Pass="'.$new_password_hash.'" WHERE Name="'.$cur_user['username'].'" ') or error('Unable to update password', __FILE__, __LINE__, $db->error());
	...
	else if (isset($_POST['form_sent']))
	{
		...
		if ($db->num_rows($result))
		{
			...
			else if ($pun_config['o_mailing_list'] != '')
			{
				...
				$db->query('UPDATE lvrp_users SET Email="'.$db->escape($new_email).'" WHERE Name="'.$cur_dupe['username'].'" ') or error('Unable to update activation data', __FILE__, __LINE__, $db->error());
...
else if ($action == 'upload_avatar' || $action == 'upload_avatar2')
{
	...
	if (isset($_POST['form_sent']))
	{
		// Easy avatar : an URL was posted
		if ($_POST['req_url'] != '')
		{	
			// Verify GD library ability to manage images (if user attempt to reach this part directly)
			if (!$gd_image_types_ability)
				message ($lang_EZavatar['Gd library unset']);
			
			// Try to retrieve the extension somewhere in the URL if it corresponds to a manageable type (not necessary, it's just to respect the source as much as possible), else the avatar will be gif type.
			$extension = '.gif';
			foreach ($possible_image_types as $cur_type => $cur_ext)
			{
				if (strpos(strtolower($_POST['req_url']), $extension = $cur_ext))	
					break;
			}
			
			// Read the file in streaming to avoid loading too large images (for security reasons)
			$open_file = @fopen($_POST['req_url'], 'rb');
			$image_bytes = '';
			if ($open_file)
			{
				while (!feof($open_file))
				{
					$image_bytes .= fread($open_file, 8192);
					if (strlen($image_bytes) > $pun_config['o_avatars_upload_size'])
					{
						fclose($open_file);
						message($lang_EZavatar['Too large remote file'].$pun_config['o_avatars_upload_size']);
					}
				}
			}
			else
				message($lang_EZavatar['Open file error']);
			fclose($open_file);
			
			// Try to create an image with the bytes previously read
			if (!$image_tmp = @imagecreatefromstring($image_bytes))
				message($lang_EZavatar['Create from string error']);
					
			// Deal with width, height and final size: reduce image if necessary
			$width = imagesx($image_tmp);
			$height = imagesy($image_tmp);
			$size = strlen($image_bytes);
			if  ($width > $pun_config['o_avatars_width'] || $height > $pun_config['o_avatars_height'] || $size > $pun_config['o_avatars_size'])
				$image_tmp = reduce_image($image_tmp);
				
			// Record the reduced image in the temporary file (to respect the source as much as possible, make it with the type corresponding to the extension possibly found in the url)
			if (!(($extension == '.gif' && @imagegif($image_tmp, PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp')) || ($extension == '.jpg' && @imagejpeg($image_tmp, PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp', 100)) || ($extension == '.png' && @imagepng($image_tmp, PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp', 0))))
			{
		...
		if (is_uploaded_file($uploaded_file['tmp_name']))
		{
			...
			// Make sure the temporary file isn't too big before moving it to the avatar directory
				if (($size = $uploaded_file['size']) > $pun_config['o_avatars_upload_size'])
					message($lang_profile['Too large'].' '.forum_number_format($pun_config['o_avatars_size']).' '.$lang_profile['bytes'].'.');

				// Move the file to the avatar directory. We do this before dealing with width, height and final size, to circumvent open_basedir restrictions
			...
			// Verify if the informations about dimensions are correct
				...
				// Easy avatar : now deal with width, height and final size: reduce image if too wide, high or large :
				if ($width > $pun_config['o_avatars_width'] || $height > $pun_config['o_avatars_height'] || $size > $pun_config['o_avatars_size'])
				{
					// Verify if the GD library is able to manage the image type
					if (!(in_array($extension, $supported_image_types)))
						message($lang_profile['Too wide or high'].' '.$pun_config['o_avatars_width'].'x'.$pun_config['o_avatars_height'].' '.$lang_profile['pixels'].'.<br />'.($lang_profile['Too large'].' '.forum_number_format($pun_config['o_avatars_size']).' '.$lang_profile['bytes'].'.'));
					
					// Create image from file
					if (!(($extension == '.gif' && ($image_tmp = @imagecreatefromgif(PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp'))) || ($extension == '.jpg' && ($image_tmp = @imagecreatefromjpeg(PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp'))) || ($extension == '.png' && ($image_tmp = @imagecreatefrompng(PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp')))))
						message($lang_EZavatar['Create image problem']);
					
					// Reduce image
					$image_tmp = reduce_image($image_tmp);
					
					// Record the reduced image in the temporary file with the right type
					if (!(($extension == '.gif' && @imagegif($image_tmp, PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp')) || ($extension == '.jpg' && @imagejpeg($image_tmp, PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp', 100)) || ($extension == '.png' && @imagepng($image_tmp, PUN_ROOT.$pun_config['o_avatars_dir'].'/'.$id.'.tmp', 0))))
					{
						imagedestroy($image_tmp);
						message($lang_profile['Move failed'].' <a href="mailto:'.$pun_config['o_admin_email'].'">'.$pun_config['o_admin_email'].'</a>.');
		...
		if ($id == 0 && $pun_user['is_admmod'])
			redirect('admin_options.php', $lang_EZavatar['Default avatar upload redirect']);
		else
	...
	if ($id == 0)
		$page_title = array(pun_htmlspecialchars($pun_config['o_board_title']), $lang_common['Profile'], $lang_EZavatar['Upload default avatar']);
	else
...
?>						
			...
			<p class="buttons"><input type="submit" name="delete_user_comply" value="<?php echo $lang_profile['Upload avatar'] ?>" /> <a href="javascript:history.go(-1)"><?php echo $lang_common['Go back'] ?></a></p>
		</form>
	</div>
</div>
<?php
...
else if (isset($_POST['delete_user']) || isset($_POST['delete_user_comply']))
{
	...
	if (isset($_POST['delete_user_comply']))
	{
		...
		$db->query('DELETE FROM lvrp_users WHERE Name='.$username);
		...
		redirect('forum.php', $lang_profile['User delete redirect']);
	}
...
else if (isset($_POST['form_sent']))
{
	...
	if ($username_updated && !isconnectedIG($old_username))
	{
		$db->query('UPDATE lvrp_users SET Name="'.$db->escape($form['username']).'" WHERE Name="'.$db->escape($old_username).'" ') or error('Unable to update thanks', __FILE__, __LINE__, $db->error());
