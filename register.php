...
if (isset($_GET['cancel']))
	redirect('forum.php', $lang_register['Reg cancel redirect']);
...
// Validate email
	...
	if (isset($_POST['origin']) && $_POST['origin'] != 0)
	{
		...
		$errors[] = 'Origine non définie';
		
	if (isset($_POST['langIG']) && $_POST['langIG'] != 0)
	{
		$langIG = $_POST['langIG'];
	}
	else
		$errors[] = 'Langue InGame non choissie';
	...
	if (empty($errors))
	{
		...
		// Add the user
		$db->query('INSERT INTO `lvrp_users` SET Name="'.$db->escape($username).'", Pass="'.$password_hash.'", Email="'.$db->escape($email1).'", Sex="'.$sex.'", Origin="'.$origin.'", Lang1="'.$langIG.'", PointsRename=1, Inscription=UNIX_TIMESTAMP(), Ip="'.get_remote_address().'" ')or error('Unable to create lvrp_user', __FILE__, __LINE__, $db->error());
		...
		redirect('forum.php', $lang_register['Reg complete']);
	}
}
			...
			<div class="inform">
				...
				<fieldset>
					...
					<div class="infldset">
						...
						<label class="required"><strong><?php echo $lang_common['Username'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /><input type="text" name="req_user" value="<?php if (isset($_POST['req_user'])) echo pun_htmlspecialchars($_POST['req_user']); ?>" size="25" maxlength="30" /><br /></label>
						<p class="clearb"><?php echo 'Votre nom doit être sous la forme Prénom_Nom'?></p>
					</div>
				</fieldset>
			</div>
...
?>
			...
			<div class="inform">
				<fieldset>
					<legend><?php echo 'Choisissez le sexe de votre personnage'; ?></legend>
					<div class="infldset">
						<label class="conl required"><?php echo 'Sexe '; ?><span><?php echo $lang_common['Required']; ?></span>
			...
			<div class="inform">
				<fieldset>
					<legend><?php echo 'Choisissez l\'origine de votre personnage';?></legend>
					<div class="infldset">
						<label class="conl required"><?php echo 'Origine ' ?><span><?php echo $lang_common['Required']; ?></span>
						<br /><select id="origin" name="origin">
							<option selected value=0>Sélectionnez</option>
			...
			<div class="inform">
				<fieldset>
					<legend><?php echo 'Choisissez une langue secondaire pour votre personnage';?></legend>
					<div class="infldset">
						<label class="conl required"><?php echo 'Langue secondaire ' ?><span><?php echo $lang_common['Required']; ?></span>
						<br /><select id="langIG" name="langIG">
						<option selected value=0>Sélectionnez</option>
						<?php $id=1;
						while($id<18)
						{
